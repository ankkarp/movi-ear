...
def frame_to_text(model, processor, path_to_frame):
    pass
...
def summarize(text):
    ...
    return summarizer(text, min_length=5, max_length=15, do_sample=False)[0]['summary_text']
...
def scene_to_text(path_to_scene_frames, translate=False):
    ...
    for frame in frames:
        text += captioner(os.path.join(path_to_scene_frames, frame))[0]['generated_text'] # .strip().capitalize() + '. '
